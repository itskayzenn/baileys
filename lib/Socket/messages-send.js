...
// Kayzen@Changes 03-02-26 --- Replace process.nextTick with PQueue for upsertMessage to avoid RSS spike... (i guess (⁠ ⁠◜⁠‿⁠◝⁠ ⁠)⁠♡)
...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            // Kayzen@Changes 02-02-26 --- Normalize message first to extract the original message and valid media type
            ...
            if (isNewsletter) {
                if (innerMessage.productMessage) {
                    extraAttrs['mediatype'] = 'image'; // Kayzen@Note 02-02-26 --- Treat product message as image message to avoid the "479" error (⁠✷⁠‿⁠✷⁠)
                }
                const patched = patchMessageBeforeSending ? await patchMessageBeforeSending(message, []) : message;
                const bytes = encodeNewsletterMessage(patched);
                // Kayzen@Changes 08-02-26 --- Add "additionalNodes" for newsletter too (⁠っ⁠˘̩⁠╭⁠╮⁠˘̩⁠)⁠っ
                ...
                binaryNodeContent.push({
                    tag: 'plaintext',
                    attrs: extraAttrs, // Kayzen@Changes 02-02-26 --- Add extraAttrs to fix media being rejected when sending to newsletter (⁠◠⁠‿⁠◕⁠)
                    content: bytes
                });
                const stanza = {
                    tag: 'message',
            ...
            // Kayzen@Changes 02-02-26 --- Add keepInChat, editedMessage, mediaNotifyMessage and pollUpdateMessage
            ...
            // Kayzen@Changes 02-02-26 --- Add native_flow_name to extraAttrs when sending interactiveResponseMessage
            ...
            // Kayzen@Changes 30-01-26 --- Add Biz Binary Node to support button messages
    ...
    const getMediaType = (message) => {
        ...
        // Kayzen@Note 02-02-26 --- Add more message type here
    ...
    return {
        ...
        // Kayzen@Changes 30-01-26 --- Add support for modifying additionalNodes and additionalAttributes using "options" in sendMessage()
            ...
            if ('disappearingMessagesInChat' in content && isJidGroup(jid)) {
                ...
                if (isDeleteMsg || isKeepMsg) {
                    ...
                    additionalNodes.push({
                        tag: 'meta',
                        attrs: {
                            // Kayzen@Note 08-02-26 --- Still a hypothesis regarding PollResult ༎ຶ⁠‿⁠༎ຶ
                ...
                // Kayzen@Changes 30-01-26 --- Add support for AI label in message when "ai" is true, but works only in private chat
                else if (isAiMsg) {
                ...
                // Kayzen@Changes 31-01-26 --- Add support for album messages
                // Kayzen@Note 06-02-26 --- Refactored to reduce high RSS usage (⁠╥⁠﹏⁠╥⁠)
                if ('album' in content) {
